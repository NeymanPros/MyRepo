...
public:
    ...
    //too many unwanted circles
    std::string d1(const Mat& c) {
        cont.clear();
        if (dotII.size() > 5) {

            if ((int)c.at<Vec3b>(dotII[0], dotII[1])[1] == 255)
                rgb = { "255", "255", "255" };
            ...
            for (int i = 2; i + 2 < angles.size(); i += 2) {
                if (angles[i] && angles[i - 2] && (angles[i - 2] > 0) == (angles[i] > 0) && (angles[i - 1] > 0) == (angles[i + 1] > 0)) {
                    if (angles[i + 1] / angles[i] > angles[i - 1] / angles[i - 2]) {
                        
                        int io = i;
                        for (; i + 2 < angles.size() && angles[i] && (angles[i + 1] / angles[i] > angles[i - 1] / angles[i - 2]) 
                            && ((angles[i - 1] / angles[i - 2] > 0) == (angles[i + 1] / angles[i] > 0)); i += 2);

                        double len = sqrt((dotII[io - 2] - dotII[i]) * (dotII[io - 2] - dotII[i]) + (dotII[io - 1] - dotII[i + 1]) * (dotII[io - 1] - dotII[i + 1]));
                        double alfa = atan(angles[io - 1] / angles[io - 2]);

                        rx = len * (cos(alfa)) / (abs(sin((double) atan(angles[io - 1] / angles[io - 2]) - atan(angles[i - 1] / angles[i - 2]))) + 0.01);
                        ry = len * (sin(alfa)) / (abs(sin((double) atan(angles[io - 1] / angles[io - 2]) - atan(angles[i - 1] / angles[i - 2]))) + 0.01);
                        
                        cw = '0';
                        
                        cont += "A " + std::to_string(rx) + ' ' + std::to_string(ry) + ' ' + std::to_string((int)(alfa * 180 / 3.1415)) +
                         " 0 " + cw + ' ' + std::to_string(dotII[i + 1]) + ' ' + std::to_string(dotII[i]) + '\n';
                        if (angles[i - 1] > 0) {
                            if (angles[i + 1] > angles[io - 1])
                                cw = '1';
                            else
                                cw = '0';
                        }
                    ...
                    else if (angles[i + 1] / angles[i] < angles[i - 1] / angles[i - 2]) {
            ...
            cont += "L " + std::to_string(dotII[dotII.size() - 3]) + ' ' + std::to_string(dotII[dotII.size() - 4]) + '\n';
            cont += "L " + std::to_string(dotII[dotII.size() - 1]) + ' ' + std::to_string(dotII[dotII.size() - 2]) + '\n';
...
int main() {
    std::string name = "C:/Users/alexe/source/repos/openCV/openCV/waves.png";
    std::cout << "\nEnter the path to the picture:\n";
    std::cin >> name;
    std::string name1 = name, result;
    ...
    for (int i = name1.length() - 1; name1[i] != '.'; i--) {
        name1.pop_back();
    }
    name1.pop_back();
    result = name1 + ".svg";
    ...
    std::ofstream newFile(name1, std::ios_base::app);
    newFile.close();
