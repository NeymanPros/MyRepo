...
impl App {
    ...
    fn view (&'_ self) -> iced::Element<'_, Message> {
        ...
        column = column.push(text(format!("Current status: {}", self.status)));
        if self.status == "Waiting" {
            let send = button("Send request").on_press(Message::GetData);
            ...
            let timer = text(format!("Waiting for respond for: {}", self.counter.load(Ordering::Relaxed)));
            
            let main_text = scrollable(text(self.text.clone().as_str().to_string()));
            
            column = column.push(timer)
                .push(text("Text to work with:").size(20).width(Fill))
                .push(main_text);
            
            let sum_text = self.container
                .iter()
                .map(|sum| return sum.lock().expect("No lock 2").clone())
                .collect::<Vec<String>>();
            ...
            for (num, scroll) in scrolls.into_iter().enumerate() {
                let mut summary = column![text(format!("{} summary:", self.sites[num])).size(20)];
            ...
            if self.status == "Done" {
                let restart = button("Do another text").on_press(Message::Restart);
...
impl Default for App {
    fn default() -> Self {
        Self {
            ...
            sites: Arc::new(["huggingface", "gemini", "cohere"]),
